...
import com.thomas.espdoorbell.doorbell.intercom.IceProperties
...
@RestController
@RequestMapping("/api/stream/ice")
class StreamingIceController(
    private val deviceService: DeviceService,
    private val sessionManager: StreamingSessionManager,
    private val mqttPublisher: MqttPublisherService,
    private val iceProperties: IceProperties
) {
    private val logger = LoggerFactory.getLogger(StreamingIceController::class.java)
    ...
    suspend fun startStream(
        ...
        logger.info("ICE stream start request from user {} for device {}", principal.id, deviceId)
        ...
        if (sessionManager.hasSession(device.deviceIdentifier)) {
            return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(StreamResponse(success = false, message = "Stream session already active"))
        }

        sessionManager.createSession(deviceId, device.deviceIdentifier, principal.id)

        val published = mqttPublisher.publishStreamStart(device.deviceIdentifier, principal.id)

        if (!published) {
            sessionManager.endSession(device.deviceIdentifier)
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(StreamResponse(success = false, message = "Failed to signal device"))
        }

        logger.info("ICE stream session started for device {}", deviceId)
    ...
    suspend fun stopStream(
        ...
        logger.info("ICE stream stop request from user {} for device {}", principal.id, deviceId)

        if (!deviceService.hasAccess(deviceId, principal.id)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                .body(StreamResponse(success = false, message = "No access"))
        }
        ...
        mqttPublisher.publishStreamStop(device.deviceIdentifier)

        logger.info("ICE stream session stopped for device {}", deviceId)
        return ResponseEntity.ok(StreamResponse(success = true))
    }
    ...
    suspend fun getEspOffer(
        ...
        val session = sessionManager.getSession(device.deviceIdentifier)
            ?: return ResponseEntity.status(HttpStatus.NOT_FOUND)
    ...
    suspend fun sendAnswer(
        ...
        val published = mqttPublisher.publishRaw(
            "doorbell/${device.deviceIdentifier}/stream/answer",
            payload
        )
        ...
        logger.info("Sent ICE answer for streaming to device {}", deviceId)
